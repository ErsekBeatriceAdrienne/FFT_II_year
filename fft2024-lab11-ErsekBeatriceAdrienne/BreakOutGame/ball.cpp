...
void Ball::gameFinished(Ball::Finisher condition)
{
    ...
    if (condition == Ball::Lost) {
        game->lost();
        //game->gameOver(); // Hozzáadott sor: Megjelenítjük a "Game Over" feliratot
    }
}

void Ball :: setXDirection(double xDir)
{
    xDirection = xDir;
}

void Ball :: setYDirection(double yDir)
{
    yDirection = yDir;
}
